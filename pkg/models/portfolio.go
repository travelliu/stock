...
type StockPortfolio struct {
	...
	Quote          *StockRealtime       `gorm:"-" json:"quote,omitempty"`
	AnalysisResult *StockAnalysisResult `gorm:"-" json:"analysisResult,omitempty"`
}

// StockPortfolioReq is the body for add/update operations.
// Code is the short numeric code (e.g. "300476"), not the full ts_code.
type StockPortfolioReq struct {
	Code   string `json:"code"`
	Note   string `json:"note,omitempty"`
	TsCode string `json:"tsCode,omitempty"`
}

func (p *StockPortfolioReq) GetCode() string {
	if p.Code != "" {
		return p.Code
	}
	if p.TsCode != "" {
